...
- 미지 역시 링크로 받아오는 형식이 아닌 페이지 디렉토리에 추가하는 방법이 있겠습니다. 하지만 성능을 위해서 그러면 서드파티를 사용하지 말아야하는가? 면 정답이 아닙니다. 본인의 서버에 사진 파일들이 많아지면, 서버에서 랜더링 요소를 줄떄 과한 비용이 발생합니다. 이미지를 링크로 받아오는 형식은 서드파티를 사용하는 부분이 있으나 이미지를 받아오는 회사들이 모두 cdn이 구축이 되어있어 이미지 요청 응답 속도가 빨라지게 됩니다. 때문에 lighthouse는 랜더, 접근, 검색 관련한 로컬 최적화이며, 실제적으로 성능 수치를 보기 위해서는 네트워크도 고려를 해야한다는 점이 있습니다.
